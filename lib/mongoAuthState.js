...
// MUHIMU: tunaweka jina la database WAZI hapa (badala ya kutegemea URI iwe
// na jina la db ndani yake). Kama MONGODB_URI yako haina "/dbName" kabla ya
// "?", baadhi ya driver za Mongo hutumia database "test" kimya kimya, au
// kutupa error - jambo linaloweza kusababisha creds/keys zisihifadhiwe vizuri
// bila kuonekana wazi kwenye logs.
const MONGODB_DB_NAME = process.env.MONGODB_DB || 'baileys_auth';
...
async function useMongoAuthState() {
  ...
  const collection = client.db(MONGODB_DB_NAME).collection(COLLECTION_NAME);
  ...
  const writeData = async (key, value) => {
    try {
      ...
      // MUHIMU: hii isipokamatwa (try/catch), Baileys hushindwa kuhifadhi
      // "keys" za ku-decrypt ujumbe unaofuata bila kuonyesha error popote -
      // dalili yake ni bot "kuonekana CONNECTED" lakini haijibu commands.
      console.error(`⚠️ Imeshindwa kuandika "${key}" MongoDB:`, e.message);
    }
  };
