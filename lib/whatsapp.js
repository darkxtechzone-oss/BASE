...
async function startSock() {
  ...
  sock = makeWASocket({
    ...
    // "silent" ilikuwa inaficha hata makosa ya ku-decrypt ujumbe (mfano
    // "keys" hazikuhifadhiwa vizuri), na hivyo bot inaonekana CONNECTED
    // lakini haijibu commands bila dalili yoyote kwenye logs. "error"
    // inaonyesha makosa muhimu tu, si "spam" ya kawaida ya connection.
  ...
  // Vitu vidogo vinavyohitajika na message.js/lib/serialize.js (mfumo mpya
  // wa kusoma commands) - havihusiani na connection/session yenyewe.
  sock.decodeJid = (jid) => {
    if (!jid) return jid;
    if (/:\d+@/gi.test(jid)) {
  ...
  // Store ndogo (kumbukumbu ya muda) inayohitajika na m.getQuotedObj kwenye
  // lib/serialize.js, ili quoted messages ziweze kupatikana.
  const store = {
    messages: new Map(),
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    if (!msg.message) {
      // Ujumbe umefika lakini "content" yake haipo - kwa kawaida ni dalili ya
      // tatizo la ku-decrypt (session/keys). Logi hii inasaidia ku-diagnose
      // badala ya bot "kunyamaza kimya" bila sababu inayoonekana.
      console.log('⚠️ Ujumbe umepokewa lakini haujasomeka (decrypt issue?) kutoka:', msg.key?.remoteJid);
      return;
    }

    // Fungua ujumbe wa "ephemeral" (disappearing messages) kabla ya kusoma.
    msg.message =
      Object.keys(msg.message)[0] === 'ephemeralMessage' ? msg.message.ephemeralMessage.message : msg.message;
    ...
    try {
      const m = await smsg(sock, msg, store);
      await messageRouter(sock, m, { messages, type }, store);
    } catch (e) {
